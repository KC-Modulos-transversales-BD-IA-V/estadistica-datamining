...
-   Varianza desconocida: Tendremos que estimar la varianza de la
    población total en base a la varianza de nuestra muestra.
...
ggplot(xdf,aes(x=z))+stat_function(fun=dnorm)+
  ...
  geom_text(x=3,y=0.1,size=4,label=paste0("n_cdf(",round(upth,2),")=",1-p/2))+
  geom_text(x=-3,y=0.1,size=4,label=paste0("n_cdf(",round(loth,2),")=",p/2))+
  theme_linedraw()
...
```{r}
p<-0.2 # 80% I.C.
error2 <- qnorm(p/2, lower.tail=F)*sm
upl2 <- m + error2
lowl2 <- m - error2
error2
```
