...
```{r}
t<-t.test(bwt[bwt$smoker=='No','Birthweight'], conf.level = 0.90)
t
```
...
```{r}
library(tidyverse)
bwt |> filter(smoker=='Yes') |> select(Birthweight) |> t.test()
```
...
```{r}
p <- 5/1000
df_covid <- data.frame(n_personas=0:1000)

df_covid$prob <- 1-dbinom(0,size=df_covid$n_personas, p=p)

ggplot(data=df_covid, aes(x=n_personas, y=prob))+geom_line(color="darkblue")
```
...
df_covid$prob_2Cont <- dbinom(2, size=df_covid$n_personas, p)

ggplot(data=df_covid, aes(x=n_personas, y=prob_2Cont))+geom_line(color="darkblue")
```
...
```{r}
df_covid$prob_2MasCont <- 1-pbinom(1, df_covid$n_personas, p)

ggplot(data=df_covid, aes(x=n_personas, y=prob_2MasCont))+geom_line(color="darkblue")+geom_line(color="red", aes(y=prob))
...
```{r}
df_covid$prob_5MasCont <- 1-pbinom(4, df_covid$n_personas, p)

ggplot(data=df_covid, aes(x=n_personas, y=prob_5MasCont))+geom_line(color="darkblue")+geom_line(color="red", aes(y=prob))
